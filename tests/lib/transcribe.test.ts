...
describe("browser transcript client", () => {
  ...
  it("uploads recorded audio for a final transcription", async () => {
    ...
    await expect(transcribe({ audio, settings })).resolves.toBe("Final transcript.");
  ...
  it("uses a filename matching the recorded audio type", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(Response.json({ transcript: "Final transcript." })));
    const audio = new Blob(["audio"], { type: "audio/mp4;codecs=mp4a.40.2" });
    const settings = { textSize: "M" as const, langFrom: "en" as const, langTo: "ja" as const, prompt: "", keywords: [] };

    await transcribe({ audio, settings });

    const [, init] = vi.mocked(fetch).mock.calls[0] as [string, RequestInit];
    expect((init.body as FormData).get("audio")).toEqual(expect.objectContaining({ name: "transcript.mp4", type: audio.type }));
  });
});
