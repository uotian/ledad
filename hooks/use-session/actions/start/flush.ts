...
import { updateTranslation } from "./translate";
...
export class Flush {
  ...
  commit() {
    ...
    if (this.sendCommit() && itemFlush) {
      void updateTranslation(itemFlush, this.langs, this.itemFlushLast, this.setItems);
    }
  }

  finalize() {
    if (this.sendCommit()) this.finalizeItem(this.langs);
  }
  ...
  private updateItem(delta: string) {
    ...
    const itemFlushNew = itemFlushCurrent
      ? { ...itemFlushCurrent, transcripts: [itemFlushCurrent.transcripts[0] + delta] as [string] }
  ...
  private finalizeItem(langs: Langs) {
    const itemFlush = this.itemFlushLast.current;
    if (itemFlush) {
      ...
      void updateTranslation(itemFlushEnded, langs, this.itemFlushLast, this.setItems);
    }
  }
}
