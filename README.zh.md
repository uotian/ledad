...
- `Text size`：S／M／L（默认为 M）。保存后立即生效，仅影响主面板内的文字大小。
- `Prompt`：输入话题或录音背景，为转写提供上下文。
- `Keywords`：输入专有名词、专业术语或缩写，每行一个，作为拼写提示。不能包含 `<` 或 `>`。

设置保存在当前浏览器中。在 Listening 状态下打开设置时，会先确认是否停止会话。点击确定会停止会话并打开设置；点击取消则继续当前会话。语言、Prompt 和 Keywords 的更改会在下次会话开始时生效。保存后，点击 Start 重新开始。
