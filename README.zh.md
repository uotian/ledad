...
- `STT Provider`：选择用于转写的 Gemini（默认）或 OpenAI。
...
监听期间，应用每30秒检查一次变化，并使用 `gpt-6-luna`（OpenAI）总结显示中的转写和翻译。摘要以单独设置的 Insights 语言显示（默认为日语）。点击 `AI Insights` 旁的刷新按钮，即使内容没有变化也可以重新生成摘要。处理期间按钮不可用；停止后仍会显示最后一次摘要。摘要仅供参考。
...
STT 使用 OpenAI 或 Gemini 的 API，翻译和 AI Insights 使用 OpenAI 的 API。
...
创建 `.env.local`，默认配置需要设置两种 API key。
