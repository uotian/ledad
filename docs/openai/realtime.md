...
このアプリでは15秒ごとに client event を送り、音声バッファを確定する。入力音声はWebSocket経由で送信する。
...
このアプリでは、15秒ごとの音声バッファ確定に対するサーバー応答として存在する。表示Itemの確定には使わない。
...
このアプリでは、15秒ごとに自動送信する。表示は引き続きdeltaをアプリ側で区切り、`completed` が来る前提にはしない。
