...
Le navigateur transmet le son du microphone à l’API OpenAI Realtime via WebRTC pour produire une transcription préliminaire à faible latence. En parallèle, il enregistre le même son par segments de 60 secondes et les envoie à `gpt-transcribe` pour produire une version finale plus précise. Les éléments préliminaires terminés sont remplacés dans la même chronologie, tandis que la partie la plus récente reste affichée comme préliminaire. La clé API OpenAI est gérée uniquement côté serveur.
